...
/** 将获取的后端路由数据格式调整成前端格式 */
export const transformBackendRoutes = (backendRoutes: Menu[] | []) => {
  return backendRoutes.map((route) => {
    // 定义一个空的 RouteRecordRaw 对象
    ...
    if (route.children?.length > 0) {
      tmpRouteItem = {
        ...
        meta: {
          ...
          elIcon: route.meta.icon,
          hidden: route.hidden
        },
        children: []
      }
      tmpRouteItem.children = transformBackendRoutes(route.children)
    } else {
      tmpRouteItem = {
        path: route.path,
        ...
        meta: {
          ...
          elIcon: route.meta.icon,
          hidden: route.hidden,
          keepAlive: route.meta.keep_alive
        }
      }
    }
    return tmpRouteItem
